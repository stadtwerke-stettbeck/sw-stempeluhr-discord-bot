...
module.exports.RunAction = async function (client, action) {
  ...
  switch (parsedAction.type) {
    ...
    case "Edit Message":
      ...
      break;
    case "Call API":
      this.CallAPI_Handle(client, parsedAction);
      break;
  }
};

module.exports.CallAPI_Handle = async function (client, action) {
  let passActions = {};
  try {
    const response = await fetch(action.url, {
      ...
      body: action.requestbody ? JSON.stringify(JSON.parse(action.requestbody)) : {},
    });

    if (response.ok) {
      const responseData = await response.json();
      let tempVariableObject = cache[action.guild.id].variables;
